...
import { useRandomAdvanced } from "../hooks/useRandomAdvanced";
...
const GENRES = [
  ...
  { id: 5, name: "Tâm Lý" },
  { id: 6, name: "Hình Sự" },
  { id: 7, name: "Chiến Tranh" },
  { id: 8, name: "Thể Thao" },
  { id: 9, name: "Võ Thuật" },
  { id: 10, name: "Viễn Tưởng" },
  { id: 11, name: "Phiêu Lưu" },
  { id: 12, name: "Khoa Học" },
  { id: 13, name: "Kinh Dị" },
  { id: 14, name: "Âm Nhạc" },
  { id: 15, name: "Thần Thoại" },
  { id: 16, name: "Chính Kịch" },
  { id: 17, name: "Học Đường" },
  { id: 18, name: "Gia Đình" },
  { id: 19, name: "Bí Ẩn" },
  { id: 20, name: "Tài Liệu" },
  { id: 21, name: "Gây Cấn" },
  { id: 22, name: "Lịch Sử" },
  { id: 23, name: "Hoạt Hình" },
  { id: 24, name: "Kiếm Hiệp" },
  { id: 25, name: "Khoa Huyễn" },
  { id: 26, name: "Chính Trị" },
  { id: 27, name: "Kinh Điển" },
  { id: 28, name: "Đời Thường" },
  { id: 29, name: "Tội Phạm" },
  { id: 30, name: "Siêu Anh Hùng" },
];

const TYPES = [
  { label: "Phim Lẻ", value: MOVIE_TYPE.PHIM_LE },
  { label: "Phim Bộ", value: MOVIE_TYPE.PHIM_BO },
  { label: "Hoạt Hình", value: MOVIE_TYPE.HOAT_HINH },
  { label: "Anime", value: MOVIE_TYPE.ANIME },
];

const STATUSES = [
  { label: "Đang Chiếu", value: MOVIE_STATUS.ONGOING },
  { label: "Hoàn Tất", value: MOVIE_STATUS.COMPLETED },
  { label: "Sắp Chiếu", value: MOVIE_STATUS.UPCOMING },
];
...
export default function RandomPage() {
  ...
  } = useRandomAdvanced(filters);
  const advMovies = advData?.data || advData || [];
  ...
  const toggleGenre = (id) => {
    setFilters((prev) => {
      ...
      if (exists)
        return { ...prev, genreIds: prev.genreIds.filter((g) => g !== id) };
        ...
        >
          ...
          <div style={{ marginBottom: 60 }}>
            ...
            >
              ...
              >
                {isFetchingQuick ? "Đang quay..." : "Quay phim khác"}
              </button>
              <span
                style={{
                  marginLeft: 16,
            ...
            {quickMovie && (
              ...
              >
                ...
                >
                  <img
                    src={quickMovie.posterUrl || FALLBACK}
                    alt={quickMovie.title}
                  ...
                  >
                    ...
                    >
                      <span style={{ color: "#f5c518", fontWeight: 700 }}>
                        ⭐ {Number(quickMovie.avgRating || 0).toFixed(1)}
                      </span>
                      <span>•</span>
                      ...
                      >
                        {MOVIE_STATUS_LABEL[quickMovie.status]}
                      </span>
                    </div>
                    <p
                      style={{
                    ...
                    >
                      {quickMovie.description?.replace(/<[^>]+>/g, "") ||
                        "Chưa có mô tả"}
                    </p>
                  </div>
                </Link>
          ...
          >
            ...
            {/* Thể loại (Full width) */}
            <FilterGroup
              title={`Thể loại (Tối đa 5 - Đã chọn: ${filters.genreIds.length})`}
              colors={colors}
            >
              {GENRES.map((g) => (
                <FilterChip
            ...
            >
              <FilterGroup title="Số lượng" colors={colors}>
                {LIMITS.map((l) => (
                  <FilterChip
                    key={l}
                    active={filters.limit === l}
                    colors={colors}
                    onClick={() => setFilters((p) => ({ ...p, limit: l }))}
                  >
                    {l} phim
                  </FilterChip>
                ))}
            ...
            {advMovies?.length > 0 && (
              ...
              >
                <h3 style={{ color: colors.headerText, marginBottom: 16 }}>
                  Kết quả:
                </h3>
                <div
                  style={{
